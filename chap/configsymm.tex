...
\paragraph{Symmetrien} Es liegen in diesem Fall nicht nur mehr Geraden vor, sondern auch deutlich mehr Symmetrien: es ist seit längerem\footcite[siehe u.\,a.][S.~102]{Tate} bekannt, dass hier die unitäre Gruppe $\Unit 4{\mathbb F_{q^2}}$ mit dem Sesquilinearprodukt definiert durch die Konjugation $\sigma \colon x \mapsto x^q$ operiert: Sei $A \in \Unit 4{\mathbb F_{q^2}}$, dann ist $(A^\top)^\sigma A = \mathbb I_4$. Offenbar können wir $\sigma$ auf $\overline{\mathbb F_p}$ fortsetzen, auch wenn es dort keine Involution mehr ist. Für einen Vektor $v \in \overline{\mathbb F_p}^4$ gilt aber immer noch $\sum v_i^{q+1} = (v^\top)^\sigma \mathbb I_4 v = (v^\top)^\sigma (A^\top)^\sigma A v = ((Av)^\top)^\sigma Av = \sum (Av)_i^{q+1}$, also lässt die unitäre Gruppe die Form $Q: v \mapsto \sum v_i^{q+1}$ auch auf Körpererweiterungen von $\mathbb F_{q^2}$ invariant.
...
\end{proof}
\begin{remarks}
Die Aussage des Korollars folgt ebenso sofort aus dem allgemeinen Satz~\ref{th:local}.
\end{remarks}
...
\paragraph{Konfiguration} Die Konfiguration der Geraden wollen wir hier nicht explizit angeben. Man kann sie allerdings indirekt angeben: wir wissen bereits, wie die Konfiguration der regulären Geraden aussieht. Bei der Stratifizierung haben wir weiterhin gesehen, dass durch Punkte in $S_2(F_d)$ nur die regulären Geraden gehen. Da nun die unitäre Gruppe auf Punkten und Geraden operiert und dabei Inzidenzrelationen erhält, und dies weiterhin auf den Punkten transitiv geschieht, können wir die Geraden durch jeden Punkt bestimmen. Inwiefern die durch die unitäre Gruppe induzierten Graphautomorphismen dann die einzigen sind, wollen wir hier nicht untersuchen.
